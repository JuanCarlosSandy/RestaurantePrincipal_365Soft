            ...
            <Dialog :visible.sync="visibleDialog" :containerStyle="dialogStyle" :modal="true" position="right" :closable="false">
                                                ...
                                                <div class="card shadow-sm">
                                                    <div class="card-body">
                                                        ...
                                                        <div class="d-flex justify-content-between mb-2">
                                                            <span><i class="fa fa-dollar mr-2"></i> Total:</span>
                ...
                <template #footer>                
                    <div class="footer-buttons">
                        ...
                        <button type="button" @click="verificarMesa()" class="btn btn-success btn-block"><i class="fa fa-check mr-2"></i> Guardar </button>
                    </div>
                </template>
            </Dialog>
...
editarMenu(event) {
    ...
    /*registrarVenta(idtipo_pago) {
        ...
        async verificarMesa() {
            ...
            console.log("El ID Mesa es: " + idMesa);

            try {
                const response = await axios.get(`/ventas/estado/${idMesa}`);
                if (response.data.tieneVenta) {
                    this.actualizarVenta(response.data.ventaId);
                } else {
                    this.registrarVenta();
                }
            } catch (error) {
        ...
        async actualizarVenta(ventaId) {
            ...
            try {
                ...
                const ventaResponse = await axios.post(`/venta/actualizar/${ventaId}`, {
                    'idcliente': null,
                    ...
                    'estado': 2,
                    'data': this.arrayDetalle,
                    'tarifaDelivery': this.tarifaDelivery,
                    'tarifaDeliveryIncluida': this.isTarifaDeliveryIncluida,
                });

                console.log("Venta actualizada exitosamente:", ventaResponse.data);
                swal(
                            'PRODUCTO AÑADIDO AL CLIENTE',
                            'Correctamente',
                            'success'
                        );
                ...
                this.$toast.add({ severity: 'success', summary: 'Exito', detail: 'Venta Actualizada Exitosamente', life: 3000 });
                this.closeDialog();
            } catch (error) {
    ...
    factura.push({
        cabecera: {
            nitEmisor: "3813665017",
            razonSocialEmisor: "MARIETA ANTEZANA GARCIA",
